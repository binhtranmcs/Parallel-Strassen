...
void mpi_2snb_strassen(int **A, int **B, int **C, int n, int m, int p, int rank) {
    ...
    //sending process
    if (rank == 0) {
        MPI_Request recv_request[6];

        MPI_Irecv(&M2[0][0], c_sub_size, MPI_INT, 1, 0, MPI_COMM_WORLD, &recv_request[0]);
        MPI_Irecv(&M4[0][0], c_sub_size, MPI_INT, 1, 0, MPI_COMM_WORLD, &recv_request[1]);
        MPI_Irecv(&M3[0][0], c_sub_size, MPI_INT, 2, 0, MPI_COMM_WORLD, &recv_request[2]);
        MPI_Irecv(&M6[0][0], c_sub_size, MPI_INT, 2, 0, MPI_COMM_WORLD, &recv_request[3]);
        MPI_Irecv(&M5[0][0], c_sub_size, MPI_INT, 3, 0, MPI_COMM_WORLD, &recv_request[4]);
        MPI_Irecv(&M7[0][0], c_sub_size, MPI_INT, 3, 0, MPI_COMM_WORLD, &recv_request[5]);
        ...
        MPI_Request send_request[14];

        //send to machine 1 to process
        MPI_Isend(&A21[0][0],a_sub_size,MPI_INT,1,0,MPI_COMM_WORLD, &send_request[0]);
        MPI_Isend(&A22[0][0],a_sub_size,MPI_INT,1,0,MPI_COMM_WORLD, &send_request[1]);
        MPI_Isend(&B11[0][0],b_sub_size,MPI_INT,1,0,MPI_COMM_WORLD, &send_request[2]);
        MPI_Isend(&B21[0][0],b_sub_size,MPI_INT,1,0,MPI_COMM_WORLD, &send_request[3]);

        //send to machine 2 to process
        MPI_Isend(&A11[0][0],a_sub_size,MPI_INT,2,0,MPI_COMM_WORLD, &send_request[4]);
        MPI_Isend(&A21[0][0],a_sub_size,MPI_INT,2,0,MPI_COMM_WORLD, &send_request[5]);
        MPI_Isend(&B11[0][0],b_sub_size,MPI_INT,2,0,MPI_COMM_WORLD, &send_request[6]);
        MPI_Isend(&B12[0][0],b_sub_size,MPI_INT,2,0,MPI_COMM_WORLD, &send_request[7]);
        MPI_Isend(&B22[0][0],b_sub_size,MPI_INT,2,0,MPI_COMM_WORLD, &send_request[8]);
        
        //send to machine 3 to process
        MPI_Isend(&A11[0][0],a_sub_size,MPI_INT,3,0,MPI_COMM_WORLD, &send_request[9]);
        MPI_Isend(&A12[0][0],a_sub_size,MPI_INT,3,0,MPI_COMM_WORLD, &send_request[10]);
        MPI_Isend(&A22[0][0],a_sub_size,MPI_INT,3,0,MPI_COMM_WORLD, &send_request[11]);
        MPI_Isend(&B21[0][0],b_sub_size,MPI_INT,3,0,MPI_COMM_WORLD, &send_request[12]);
        MPI_Isend(&B22[0][0],b_sub_size,MPI_INT,3,0,MPI_COMM_WORLD, &send_request[13]);

        for (int i = 0; i < 14; ++i) MPI_Wait(&send_request[i], MPI_STATUS_IGNORE);
        ...
        for (int i = 0; i < 6; ++i) MPI_Wait(&recv_request[i], MPI_STATUS_IGNORE);
